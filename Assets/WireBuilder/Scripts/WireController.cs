...
public class WireController : MonoBehaviour
{
    ...
    public void DistanceBetweenStartAndEnd()
    {
        ...
        if (currentDistanceToStartAnchor > maxDistanceToStarAnchor)
        {
            ...
            /// </summary>>
            if (endAnchorTemp.gameObject.TryGetComponent<Grabbable>(out Grabbable grabbableObj))
            {
                grabbableObj.Drop();
                endAnchorTemp.gameObject.GetComponent<Rigidbody>().AddForce(Vector3.Normalize(startAnchorPoint.position - endAnchorTemp.position) * 5f + Vector3.up * 2f, ForceMode.Impulse);
            }
        }
    }
